...
const previousCallBack = (error, output, input) => {
  if (error) {
    throw error;
  } else {
    branches[1] = output.trim();
    console.log('Current branch is ', branches[1]);
    executeCommand(branches[0], branches[1]);
  }
};
  ...
  exec(
    ...
    (error, output, input) => {
      if (error) {
        ...
        console.log('previous Branch is ', branches[0]);
        exec(`git symbolic-ref HEAD 2>/dev/null | cut -d"/" -f 3,4,5`, previousCallBack);
      }
    },
  );
}
